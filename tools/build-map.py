...
out.append("")
out.append("// Inverse projection: normalised map point (x,y) back to real lng/lat.")
out.append("// x,y are 0..1 fractions of this viewBox; used for the on-map coordinate readout.")
out.append("export const PROJ = {")
out.append(f"    lng0: {LNG0:.6f},")
out.append(f"    lngSpan: {LNG1 - LNG0:.6f},")
out.append(f"    lat0: {LAT0:.6f},")
out.append(f"    latSpan: {LAT0 - LAT1:.6f},")
out.append("} as const;")
out.append("")
out.append("export function pointToLatLng(x: number, y: number): { lat: number; lng: number } {")
out.append("    return { lat: PROJ.lat0 - y * PROJ.latSpan, lng: PROJ.lng0 + x * PROJ.lngSpan };")
out.append("}")
out.append("")
out.append("/** Field-guide style coordinate string, e.g. \"24.99\u00b0 S \u00b7 31.55\u00b0 E\". */")
out.append("export function formatLatLng(x: number, y: number): string {")
out.append("    const { lat, lng } = pointToLatLng(x, y);")
out.append("    const ns = lat < 0 ? \"S\" : \"N\";")
out.append("    const ew = lng < 0 ? \"W\" : \"E\";")
out.append("    return `${Math.abs(lat).toFixed(2)}\u00b0 ${ns} \u00b7 ${Math.abs(lng).toFixed(2)}\u00b0 ${ew}`;")
out.append("}")
out.append("")
