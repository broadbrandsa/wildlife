...
- **Fonts:** `var(--font-serif)` (Spectral) for display/headings and clue text; `var(--font-sans)` (Hanken Grotesk) for UI/body; `var(--font-mono)` (Space Mono) for eyebrows, data, counts, codes — UPPERCASE with wide tracking.
...
- **Icons:** Phosphor web font — `<i className="ph ph-paw-print" />`, `-fill` variants for active states.
